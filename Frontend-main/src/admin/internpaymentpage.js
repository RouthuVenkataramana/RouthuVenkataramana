...
export default function Paymentpage() {
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_ENQUIRY}/getinternpaymentshistory/${Id}`).then((res)=>{
      if(res.status===200){
    ...
    if (open) {
      axios.get(`${process.env.REACT_APP_ENQUIRY}/getinternstudentforpayment/${Id}`)
        .then((res) => {
          if (res.status === 200) {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(`${process.env.REACT_APP_ENQUIRY}/createinternpayment`, details)
      .then((res) => {
        if (res.status === 200) {
          Swal.fire({
            position: "center",
    ...
    axios.post(`${process.env.REACT_APP_ENQUIRY}/updatefee/${Id}`, updatedFeeDetails)
      .then((res) => {
        if (res.status === 200) {
          Swal.fire({
            position: "center",
